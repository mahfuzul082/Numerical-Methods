function[x_, fx_, n_itr, err_abs] = secant(f, x_0, x_00, tlr, n_max)

%initialize output arguments
x_= []; fx_= []; n_itr = []; err_abs = [];

for i=1:n_max
    if feval(f, x_00) - feval(f, x_0) == 0
        fprintf("Division by zero. Choose a different initial guess, or method.\n");
        break;
    else
        x_ = [x_; x_00]; %update the array for iterate
        x_1 = x_00 - (feval(f, x_00).*(x_00 - x_0))/(feval(f, x_00) - feval(f, x_0)); %secant iteration formula
        fx_ = [fx_; x_1]; %update the array for function
        n_itr = [n_itr; i]; %update the array for no. of iterations
        err_abs = [err_abs; abs(x_1 - x_00)]; %update the array for absolute error
