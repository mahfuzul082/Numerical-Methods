function[x_, fx_, n_itr, err_abs] = false_position(f, a, b, tlr, n_max)
...
for i=1:n_max
    ...
    elseif (feval(f, b) - feval(f, a)) == 0
        fprintf("Division by zero. Choose different initial guesses or method.\n");
        break;
    else
        x_0 = b - feval(f, b)*(b - a)./(feval(f, b) - feval(f, a)); %false position iteration formula
        ...
        err_abs = [err_abs; abs(b - x_0)]; %update the array for absolute error
        fprintf("After iteration no. %d, x_%d = %0.9f \n", i, i, x_0)
        if abs(b - x_0) < tlr
